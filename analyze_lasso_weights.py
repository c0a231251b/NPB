...
def main():
    ...
    # Pattern B (打者タイプ分類) でデータをロード
    X, y = load_dataset("B", json_dir, manager)
    ...
    feature_names = []
    # 打者1番〜9番 (各6次元)
    types = ['Hand', 'Power', 'Average', 'Speed', 'Eye', 'All-Rounder']
    for i in range(1, 10):
        for t in types:
            feature_names.append(f"{i}番_{t}")
    
    # 投手 (7次元)
    p_cols = ['P_hand', 'P_ERA', 'P_K/9', 'P_HR/9', 'P_Fast_G', 'P_Break_G', 'P_Fall_G']
    feature_names.extend(p_cols)
    ...
    # 有効な（0でない）特徴量だけを抽出してソート
    active_weights = weights[weights['Weight'] != 0].copy()
    active_weights['Abs_Weight'] = active_weights['Weight'].abs()
    active_weights = active_weights.sort_values(by='Abs_Weight', ascending=False)

    print("\n" + "="*50)
    print("Lasso回帰：有効な特徴量ランキング (TOP 23)")
    print("="*50)
    print(f"有効な特徴量数: {len(active_weights)} / 61")
    print("-" * 50)
    
    for _, row in active_weights.iterrows():
        impact = "プラス貢献" if row['Weight'] > 0 else "マイナス影響"
        print(f"{row['Feature']:20} | 係数: {row['Weight']:7.4f} | {impact}")
    print("="*50)
