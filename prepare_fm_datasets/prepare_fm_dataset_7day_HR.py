#============================================================
# prepare_fm_dataset_7day.pyの派生、prepare_fm_dataset_7day_HR.pyを作成
# ホームラン率 - 被ホームラン率（7日間）を特徴量として使用するバージョン
# 目的変数：その試合の得点
# 出力：fm_dataset_7d_HR.pkl
#============================================================
...
# メイン処理（7day HR 専用）
#============================================================
def prepare_fm_dataset_7d_HR():

    with open("player_id_master.json", "r", encoding="utf-8") as f:
        master = json.load(f)

    # ★ HR 版の JSON ディレクトリ
    JSON_DIR_7D = "game_data_2025_match_results_add_7day_stats_HR"
    ...
    print(f"--- 7day HR データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    # 出力
    #============================================================
    with open("matching_failed_7d_HR.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_7d_HR.pkl")
    print("完了! 'fm_dataset_7d_HR.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_7d_HR()
