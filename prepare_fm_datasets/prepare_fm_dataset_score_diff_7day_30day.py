...
def prepare_fm_dataset():
    ...
    for path in json_paths:
        ...
        for t_name in teams:
            ...
            dataset.append({
                ...
                "target": float(scores[t_name]-scores[opp_t]) # 得点差(自チーム得点ー相手チームt得点)を目的変数にする
            })
    ...
    with open(f"matching_failed_score_diff_7d_30d.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(list(set(failed_names)))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_score_diff_7d_30d.pkl")
    print(f"完了! ID 0のリストを 'matching_failed_score_diff_7d_30d.txt' に書き出しました。")
