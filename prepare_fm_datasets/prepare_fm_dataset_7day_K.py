#============================================================
# prepare_fm_dataset_7day.pyの派生、prepare_fm_dataset_7day_K.pyを作成
# K% - 被K%（7日間）を特徴量として使用するバージョン
# 目的変数：その試合の得点
# 出力：fm_dataset_7d_K.pkl
#============================================================
...
def prepare_fm_dataset_7d_K():
    ...
    print(f"--- 7day K% データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    # 出力
    #============================================================
    with open("matching_failed_7d_K.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_7d_K.pkl")
    print("完了! 'fm_dataset_7d_K.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_7d_K()
