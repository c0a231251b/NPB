...
#============================================================
# メイン処理（7day 専用・score_diff）
#============================================================
def prepare_fm_dataset_7d_score_diff():
    ...
    print(f"--- 7day 専用データセット生成（score_diff）: {len(json_paths)}試合を処理中 ---")
    ...
    for path in json_paths:
        ...
        for t_name in teams:
            ...
            # 得点差（自チーム − 相手チーム）
            score_diff = float(scores[t_name] - scores[opp_t])
    ...
    # 出力
    #============================================================
    with open("matching_failed_score_diff_7d.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_score_diff_7d.pkl")
    print("完了! 'fm_dataset_score_diff_7d.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_7d_score_diff()
