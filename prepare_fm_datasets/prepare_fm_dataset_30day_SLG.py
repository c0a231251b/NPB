#============================================================
# prepare_fm_dataset_30day.pyの派生、prepare_fm_dataset_30day_SLG.pyを作成
# 長打率 - 被長打率（30日間）を特徴量として使用するバージョン
# 目的変数：その試合の得点
# 出力：fm_dataset_30d_SLG.pkl
#============================================================
...
# メイン処理（30day SLG 専用）
#============================================================
def prepare_fm_dataset_30d_SLG():

    with open("player_id_master.json", "r", encoding="utf-8") as f:
        master = json.load(f)

    # ★ SLG 版の JSON ディレクトリ
    JSON_DIR_30D = "game_data_2025_match_results_add_30day_stats_SLG"
    ...
    print(f"--- 30day SLG データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    # 出力
    #============================================================
    with open("matching_failed_30d_SLG.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_30d_SLG.pkl")
    print("完了! 'fm_dataset_30d_SLG.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_30d_SLG()
