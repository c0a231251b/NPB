#=====
# 元ファイル：prepare_fm_dataset_7day_30day.py
# ファイル名： prepare_fm_dataset_7day.py
# 7dayのみの特徴量を使うバージョン
#=====
...
def prepare_fm_dataset_7d():
    ...
    print(f"--- 7day 専用データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    # 出力
    #============================================================
    with open("matching_failed_7d.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_7d.pkl")
    print("完了! 'fm_dataset_7d.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_7d()
