#============================================================
# prepare_fm_dataset_30day.pyの派生、prepare_fm_dataset_30day_OBP.pyを作成
# 出塁率 - 被出塁率（30日間）を特徴量として使用するバージョン
# 目的変数：その試合の得点
# 出力：fm_dataset_30d_OBP.pkl
#============================================================
...
# メイン処理（30day OBP 専用）
#============================================================
def prepare_fm_dataset_30d_OBP():
    ...
    print(f"--- 30day OBP データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    # 出力
    #============================================================
    with open("matching_failed_30d_OBP.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_30d_OBP.pkl")
    print("完了! 'fm_dataset_30d_OBP.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_30d_OBP()
