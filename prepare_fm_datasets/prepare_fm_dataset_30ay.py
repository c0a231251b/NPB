...
# ファイル名： prepare_fm_dataset_30day.py
# 7dayのみの特徴量を使うバージョン
#=====
#============================================================
# インポート
...
def prepare_fm_dataset_30d():
    ...
    print(f"--- 30day 専用データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    # 出力
    #============================================================
    with open("matching_failed_30d.txt", "w", encoding="utf-8") as f:
        if failed_names:
            f.write("\n".join(sorted(set(failed_names))))
        else:
            f.write("すべての選手が特定されました！")

    pd.DataFrame(dataset).to_pickle("fm_dataset_30d.pkl")
    print("完了! 'fm_dataset_30d.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_30d()
