...
def perform_clustering():
    ...
    model.load_state_dict(torch.load("fm_model_score_diff.pth"), strict=False)
    ...
    plt.tight_layout()
    plt.savefig(f"clustering_metrics_evaluation_K=4_score_diff.png")
    plt.close()
    print(f"投手を除外した、打者限定の左右併記グラフを保存しました: clustering_metrics_evaluation_K=4_score_diff.png")

    # --- 4. 【暫定】数理の正解である K=4 で一度クラスタリングを実行し保存 ---
    """
    注意！！！！！！！
    ここを変更
    """
    ...
    df_res.to_csv(f"extracted_player_clusters_K{chosen_k}_score_diff.csv", index=False, encoding="utf-8-sig")
    print(f"打者のみにクラスターIDを付与して保存しました: extracted_player_clusters_K{chosen_k}_score_diff.csv")
