...
# 30日間 HR% - 被HR% の contrast1〜contrast9 を特徴量として使用
...
#============================================================
def prepare_fm_dataset_30d_HR_contrast9():

    with open("player_id_master.json", "r", encoding="utf-8") as f:
        master = json.load(f)

    JSON_DIR = "game_data_2025_match_results_add_30day_stats_HR_contrast9"
    ...
    for path in json_paths:
        ...
        #============================================================
        # contrast1〜contrast9 を取得
        #============================================================
        team_contrast = {teams[0]: [], teams[1]: []}
        seen = set()
        ...
        for ab in data.get("at_bat_features", []):
            ...
            # contrast1〜contrast9 をそのまま使う
            contrasts = []
            for i in range(1, 10):
                val = ab.get(f"contrast{i}", 0.0)
                contrasts.append(float(val))

            team_contrast[team].append(contrasts)

        #============================================================
        # ID 解決
        #============================================================
        def resolve_id(t, s, role):
            if not s:
                return 0
            ...
            if pid == 0:
                failed_names.append(f"{role}: {t} {s}")
            return pid
        ...
        for t_name in teams:
            ...
            # contrast9 特徴量
            c_list = team_contrast[t_name]
            if not c_list:
                c_list = [[0.0] * 9]

            # 各打者の contrast9 を平均化
            contrast_features = list(np.mean(np.array(c_list), axis=0))

            dataset.append({
                "cat_features": b_ids + [p_id],
                "num_features": contrast_features,
    ...
    # 出力
    #============================================================
    with open("matching_failed_30d_HR_contrast9.txt", "w", encoding="utf-8") as f:
        if failed_names:
...
if __name__ == "__main__":
    prepare_fm_dataset_30d_HR_contrast9()
