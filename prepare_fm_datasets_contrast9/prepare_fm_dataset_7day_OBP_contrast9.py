#============================================================
# prepare_fm_dataset_7day_OBP_contrast9.py
# 出塁率 - 被出塁率（7日間）の対比特徴量を
# contrast1〜contrast9 としてそのまま出力する版
# 出力：fm_dataset_7d_OBP_contrast9.pkl
#============================================================
...
# メイン処理（7day OBP 専用）
#============================================================
def prepare_fm_dataset_7day_OBP_contrast9():

    with open("player_id_master.json", "r", encoding="utf-8") as f:
        master = json.load(f)

    JSON_DIR_7D = "game_data_2025_match_results_add_7day_stats_OBP"
    ...
    json_paths = glob.glob(os.path.join(JSON_DIR_7D, "*.json"))
    print(f"--- 7day OBP contrast9 データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    for path in json_paths:
        ...
        #============================================================
        # 7day OBP 対比特徴量（contrast1〜contrast9）
        #============================================================
        team_contrast7 = {teams[0]: [], teams[1]: []}
        seen = set()
    ...
    # 出力
    #============================================================
    pd.DataFrame(dataset).to_pickle("fm_dataset_7d_OBP_contrast9.pkl")
    print("完了! 'fm_dataset_7d_OBP_contrast9.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_7day_OBP_contrast9()
