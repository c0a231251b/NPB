#============================================================
# prepare_fm_dataset_30day_HR_contrast9.py
# HR% - 被HR%（30日間）の対比特徴量（contrast1〜contrast9）を
# そのまま LightGBM 用に出力するバージョン
# 出力：fm_dataset_30d_HR_contrast9.pkl
#============================================================
...
#============================================================
# メイン処理（30day HR contrast9）
#============================================================
def prepare_fm_dataset_30day_HR_contrast9():

    with open("player_id_master.json", "r", encoding="utf-8") as f:
        master = json.load(f)

    JSON_DIR = "game_data_2025_match_results_add_30day_stats_HR"

    name_to_id = master["name_to_id"]
    norm_id_map = {aggressive_normalize(k): v for k, v in name_to_id.items()}
    ...
    print(f"--- 30day HR contrast9 データセット生成: {len(json_paths)}試合を処理中 ---")
    ...
    for path in json_paths:
        ...
        # スコア
        scores = {
            standardize_team(e["team"]): int(aggressive_normalize(e.get("R", "0")))
        ...
        for entry in data.get("text_live", []):
            ...
            for play in entry.get("plays", []):
                for line in play.get("lines", []):
                    ...
                    # 打順
                    m = re.search(r"([1-9])番\s*(\S+)", line)
                    if m:
                        order = int(m.group(1))
                        short = aggressive_normalize(m.group(2))
                    ...
                    # 投手
                    pm = re.search(r"投[手]?[：\s]+(\S+)", line)
                    if pm:
                        p_name = aggressive_normalize(pm.group(1))
                        if lineup_detect[pit_t][0] is None:
                            lineup_detect[pit_t][0] = p_name

        #============================================================
        # HR contrast1〜contrast9 特徴量
        #============================================================
        team_contrast9 = {teams[0]: [], teams[1]: []}
        seen = set()
        ...
        for ab in data.get("at_bat_features", []):
            ...
            # contrast1〜contrast9 をそのまま取り出す
            contrasts = []
            for i in range(1, 10):
                val = ab.get(f"contrast{i}", 0.0)
                contrasts.append(float(val))

            team_contrast9[team].append(contrasts)
        ...
        def resolve_id(t, s, role_label):
            ...
            if not full:
                for nm in norm_id_map.keys():
                    if s in nm:
                        full = nm
                        break

            pid = norm_id_map.get(full, 0)
            if pid == 0:
        ...
        for t_name in teams:
            ...
            # contrast9 特徴量
            c_list = team_contrast9[t_name]
            if not c_list:
                c_list = [[0.0] * 9]

            # 各 contrast の平均
            contrast_means = list(np.mean(c_list, axis=0))
            ...
            dataset.append({
                ...
                "pitcher": p_id,
                "contrast1": contrast_means[0],
                "contrast2": contrast_means[1],
                "contrast3": contrast_means[2],
                "contrast4": contrast_means[3],
                "contrast5": contrast_means[4],
                "contrast6": contrast_means[5],
                "contrast7": contrast_means[6],
                "contrast8": contrast_means[7],
                "contrast9": contrast_means[8],
    ...
    # 出力
    #============================================================
    pd.DataFrame(dataset).to_pickle("fm_dataset_30d_HR_contrast9.pkl")
    print("完了! 'fm_dataset_30d_HR_contrast9.pkl' を生成しました。")


if __name__ == "__main__":
    prepare_fm_dataset_30day_HR_contrast9()
