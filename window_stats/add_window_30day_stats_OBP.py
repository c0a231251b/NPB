...
- game_data_2025_match_results_add_30day_stats_OBP/  (出力)

このスクリプトを実行するコマンドは以下の通り
python add_window_30day_stats_OBP.py --output_dir game_data_2025_match_results_add_30day_stats_OBP game_data_2025_match_results
"""
...
class RollingStats:
        ...
        for ab in at_bats:
            if ab['batter']:
                self.batter_log[ab['batter']].append(
                    (game_date, ab['is_ab'], ab['is_hit'], ab['is_bb'], ab['is_hbp'], ab['is_sf'])
                )
            if ab['pitcher']:
    ...
    def _calc_obp(self, log, before_date):
        ...
        for (d, is_ab, hit, bb, hbp, sf) in log:
    ...
    def batter_obp(self, name, game_date):
        return self._calc_obp(self.batter_log.get(name, []), game_date)

    def pitcher_obp(self, name, game_date):
        return self._calc_obp(self.pitcher_log.get(name, []), game_date)
...
def process(json_dir, output_dir=None, inplace=False):
    ...
    for i, (game_date, filepath, data) in enumerate(game_logs):
        ...
        for ab in at_bats:
            ...
            if use_stats:
                b_obp = rolling.batter_obp(ab['batter'], game_date)
                p_obp = rolling.pitcher_obp(ab['pitcher'], game_date) if ab['pitcher'] else None

                if b_obp is not None and p_obp is not None:
                    contrast = round(b_obp - p_obp, 4)
                else:
                    contrast = 0.0
            else:
                b_obp = None
            ...
            features.append({
                ...
                'batter_obp_30d': b_obp,
                'pitcher_obp_30d': p_obp,
