...
打率-被打率の差分を特徴量として付与するバージョンから三振率-奪三振率の差分を特徴量として付与するバージョンに変更
その他実装はadd_window_30day_stats.pyと同じ
- game_data_2025_match_results/  (入力)
- game_data_2025_match_results_add_30day_stats_K/  (出力)

このスクリプトを実行するコマンドは以下の通り
python add_window_30day_stats_K.py --output_dir game_data_2025_match_results_add_30day_stats_K game_data_2025_match_results
"""


"""
add_window_30day_stats.py の K% 版
三振率 - 奪三振率 を特徴量として付与する
"""
...
def is_strikeout(result: str) -> bool:
    return ("三振" in result) or ("振逃" in result)

# 打席判定（投手行以外はすべて PA として扱う）
def is_plate_appearance(result: str) -> bool:
    return True  # 打席結果はすべて PA としてカウント
...
class RollingStats:
    ...
    def _calc_k_rate(self, log, before_date):
        cutoff = before_date - timedelta(days=self.window)

        PA = 0
        K = 0

        for (d, is_pa, is_k) in log:
    ...
    def batter_k_rate(self, name, game_date):
        return self._calc_k_rate(self.batter_log.get(name, []), game_date)

    def pitcher_k_rate(self, name, game_date):
        return self._calc_k_rate(self.pitcher_log.get(name, []), game_date)
...
def process(json_dir, output_dir=None, inplace=False):
    ...
    for i, (game_date, filepath, data) in enumerate(game_logs):
        ...
        for ab in at_bats:
            ...
            if use_stats:
                b_k = rolling.batter_k_rate(ab['batter'], game_date)
                p_k = rolling.pitcher_k_rate(ab['pitcher'], game_date) if ab['pitcher'] else None

                if b_k is not None and p_k is not None:
                    contrast = round(b_k - p_k, 4)
                else:
                    contrast = 0.0
            else:
                b_k = None
