...
打率-被打率の差分を特徴量として付与するバージョンから四球率-与四球率の差分を特徴量として付与するバージョンに変更
その他実装はadd_window_7day_stats.pyと同じ
- game_data_2025_match_results/  (入力)
- game_data_2025_match_results_add_30day_stats_BB/  (出力)

このスクリプトを実行するコマンドは以下の通り
python add_window_30day_stats_BB.py --output_dir game_data_2025_match_results_add_30day_stats_BB game_data_2025_match_results
"""

"""
add_window_7day_stats.py の BB% 版
四球率 - 与四球率 を特徴量として付与する
"""
...
def is_bb(result: str) -> bool:
    return ("四球" in result) or ("敬遠" in result)
...
class RollingStats:
    ...
    def _calc_bb_rate(self, log, before_date):
        cutoff = before_date - timedelta(days=self.window)

        PA = 0
        BB = 0

        for (d, is_pa, is_bb) in log:
        ...
        return round(BB / PA, 4)

    def batter_bb_rate(self, name, game_date):
        return self._calc_bb_rate(self.batter_log.get(name, []), game_date)

    def pitcher_bb_rate(self, name, game_date):
        return self._calc_bb_rate(self.pitcher_log.get(name, []), game_date)
...
def process(json_dir, output_dir=None, inplace=False):
    ...
    for i, (game_date, filepath, data) in enumerate(game_logs):
        ...
        for ab in at_bats:
            ...
            if use_stats:
                b_bb = rolling.batter_bb_rate(ab['batter'], game_date)
                p_bb = rolling.pitcher_bb_rate(ab['pitcher'], game_date) if ab['pitcher'] else None

                if b_bb is not None and p_bb is not None:
                    contrast = round(b_bb - p_bb, 4)
                else:
                    contrast = 0.0
            else:
                b_bb = None
