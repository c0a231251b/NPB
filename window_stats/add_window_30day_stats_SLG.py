...
打率-被打率の差分を特徴量として付与するバージョンから長打率-被長打率の差分を特徴量として付与するバージョンに変更
その他実装はadd_window_30day_stats.pyと同じ
- game_data_2025_match_results/  (入力)
- game_data_2025_match_results_add_30day_stats_SLG/  (出力)

このスクリプトを実行するコマンドは以下の通り
python add_window_30day_stats_SLG.py --output_dir game_data_2025_match_results_add_30day_stats_SLG game_data_2025_match_results
"""
...
class RollingStats:
    ...
    def batter_slg(self, name, game_date):
        return self._calc_slg(self.batter_log.get(name, []), game_date)

    def pitcher_slg(self, name, game_date):
        return self._calc_slg(self.pitcher_log.get(name, []), game_date)
...
def process(json_dir, output_dir=None, inplace=False):
    ...
    for i, (game_date, filepath, data) in enumerate(game_logs):
        ...
        for ab in at_bats:
            ...
            if use_stats:
                b_slg = rolling.batter_slg(ab['batter'], game_date)
                p_slg = rolling.pitcher_slg(ab['pitcher'], game_date) if ab['pitcher'] else None

                if b_slg is not None and p_slg is not None:
                    contrast = round(b_slg - p_slg, 4)
                else:
                    contrast = 0.0
            else:
                b_slg = None
            ...
            features.append({
                ...
                'batter_slg_30d': b_slg,
                'pitcher_slg_30d': p_slg,
