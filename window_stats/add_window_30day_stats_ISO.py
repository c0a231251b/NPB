"""
add_window_30day_stats.pyの派生スクリプト
打率-被打率の差分を特徴量として付与するバージョンからISO-被ISOの差分を特徴量として付与するバージョンに変更
その他実装はadd_window_30day_stats.pyと同じ
- game_data_2025_match_results/  (入力)
- game_data_2025_match_results_add_30day_stats_ISO/  (出力)

このスクリプトを実行するコマンドは以下の通り
python add_window_30day_stats_ISO.py --output_dir game_data_2025_match_results_add_30day_stats_ISO game_data_2025_match_results
"""

"""
add_window_30day_stats.py の ISO 版
ISO（純長打率） - 被ISO を特徴量として付与する
"""
...
def is_hit(result: str) -> bool:
    return ("安" in result) or ("２" in result) or ("３" in result) or ("本" in result)
...
class RollingStats:
    ...
    def _calc_iso(self, log, before_date):
        ...
        for (d, is_ab, is_hit, bases) in log:
    ...
    def batter_iso(self, name, game_date):
        return self._calc_iso(self.batter_log.get(name, []), game_date)

    def pitcher_iso(self, name, game_date):
        return self._calc_iso(self.pitcher_log.get(name, []), game_date)
...
def process(json_dir, output_dir=None, inplace=False):
    ...
    for i, (game_date, filepath, data) in enumerate(game_logs):
        ...
        for ab in at_bats:
            ...
            if use_stats:
                b_iso = rolling.batter_iso(ab['batter'], game_date)
                p_iso = rolling.pitcher_iso(ab['pitcher'], game_date) if ab['pitcher'] else None

                if b_iso is not None and p_iso is not None:
                    contrast = round(b_iso - p_iso, 4)
                else:
                    contrast = 0.0
            else:
                b_iso = None
            ...
            features.append({
                ...
                'batter_iso_30d': b_iso,
                'pitcher_iso_30d': p_iso,
