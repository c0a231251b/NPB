# -*- coding: utf-8 -*-
"""
Step 2: LightGBM inning-level RUN_SCORED prediction with lineup-strength features.
...
実行例:
python evaluate_step2_lightgbm_lineup_strength.py --inning_csv_path "C:/Users/Admin/Desktop/NPB/evaluate_inning/data/train_inning_runs_added_9_features.csv" --pa_csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv"
python evaluate_step2_lightgbm_lineup_strength.py \
...
def add_lineup_strength_features(inning_df: pd.DataFrame, pa_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    validate_required_columns(
        pa_df,
        ["game_date", "stadium_id", "top_bottom", "bat_order"] + PA_LINEUP_VALUE_COLUMNS,
        "PA CSV",
    )
    validate_required_columns(
        inning_df,
        ["game_date", "stadium_id", "team_id", "top_bottom", "batting_order_start"],
        "inning CSV",
    )
    ...
    for row in df.itertuples(index=False):
        ...
        start = int(row.batting_order_start)
        ...
        for order in next_orders(start, 4):
            key = (game_date, stadium_id, tb, order)
            val = lookup.get(key)
            if val is None:
                if order in next_orders(start, 3):
    ...
    for col in LINEUP_STRENGTH_NUMERIC_FEATURES + LINEUP_DIAGNOSTIC_FEATURES:
...
def prepare_inning_dataframe(inning_csv_path: Path, pa_csv_path: Path, output_dir: Path, save_augmented_csv: bool = True) -> pd.DataFrame:
    ...
    numeric_features = BASE_NUMERIC_FEATURES + SCORE_CONTEXT_NUMERIC_FEATURES + LINEUP_STRENGTH_NUMERIC_FEATURES + LINEUP_DIAGNOSTIC_FEATURES
    ...
    lineup_table.to_csv(output_dir / "step2_lineup_slot_table_from_pa.csv", index=False, encoding="utf-8-sig")

    if save_augmented_csv:
        df.to_csv(output_dir / "train_inning_runs_step2_lineup_strength.csv", index=False, encoding="utf-8-sig")
...
def get_feature_variants() -> List[FeatureVariant]:
    ...
    return [
        ...
        FeatureVariant(
            ...
            description="Step2: score context + 次の3/4打者の能力特徴量。",
            categorical_features=score_cat,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_step2_lightgbm_lineup_strength",
        help="出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("Step 2: LightGBM lineup-strength feature comparison")
    print("=" * 80)
    ...
    coverage_df.to_csv(output_dir / "step2_lineup_feature_coverage.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(summary_rows).to_csv(output_dir / "step2_feature_variant_summary.csv", index=False, encoding="utf-8-sig")
    ...
    comparison_df.to_csv(output_dir / "step2_lightgbm_lineup_strength_comparison_metrics.csv", index=False, encoding="utf-8-sig")

    print("\n" + "=" * 80)
    print("Step 2 comparison")
    print("=" * 80)
    print(comparison_df.to_string(index=False))
    print(f"\nSaved: {output_dir / 'step2_lightgbm_lineup_strength_comparison_metrics.csv'}")
    print("Done.")
