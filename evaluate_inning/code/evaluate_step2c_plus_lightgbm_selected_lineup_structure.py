# -*- coding: utf-8 -*-
"""
Step 2C+: LightGBM inning-level RUN_SCORED prediction with selected enhanced lineup-structure features.
...
実行例:
python evaluate_step2c_plus_lightgbm_selected_lineup_structure.py --inning_csv_path "C:/Users/Admin/Desktop/NPB/evaluate_inning/data/train_inning_runs_added_9_features.csv" --pa_csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv"
python evaluate_step2c_plus_lightgbm_selected_lineup_structure.py \
...
# Step2C+ features: Step2A + selected high-importance leadoff / chain / role / variance-max lineup structure
STEP2C_PLUS_ADDED_NUMERIC_FEATURES = [
    # 1. 先頭打者特徴量（Step2C重要度上位から選択）
...
# Step2Cで作成するが、Step2C+モデルでは使わない追加特徴量。
# coverageや拡張CSVには残すため、診断用として保持する。
STEP2C_UNUSED_NUMERIC_FEATURES = [
...
STEP2C_ALL_ENGINEERED_NUMERIC_FEATURES = STEP2C_PLUS_ADDED_NUMERIC_FEATURES + STEP2C_UNUSED_NUMERIC_FEATURES
...
def add_lineup_strength_features(inning_df: pd.DataFrame, pa_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    all_added = (
        LINEUP_SUPPORT_NUMERIC_FEATURES
        + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES
        + STEP2C_ALL_ENGINEERED_NUMERIC_FEATURES
        + LINEUP_DIAGNOSTIC_FEATURES
    )
...
def prepare_inning_dataframe(inning_csv_path: Path, pa_csv_path: Path, output_dir: Path, save_augmented_csv: bool = True) -> pd.DataFrame:
    ...
    numeric_features = BASE_NUMERIC_FEATURES + SCORE_CONTEXT_NUMERIC_FEATURES + LINEUP_SUPPORT_NUMERIC_FEATURES + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES + STEP2C_ALL_ENGINEERED_NUMERIC_FEATURES + LINEUP_DIAGNOSTIC_FEATURES
    ...
    lineup_table.to_csv(output_dir / "step2c_plus_lineup_slot_table_from_pa.csv", index=False, encoding="utf-8-sig")

    if save_augmented_csv:
        df.to_csv(output_dir / "train_inning_runs_step2c_plus_selected_lineup_structure.csv", index=False, encoding="utf-8-sig")
...
def get_feature_variants() -> List[FeatureVariant]:
    ...
    step2c_plus_num = unique_features(score_num + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES + STEP2C_PLUS_ADDED_NUMERIC_FEATURES)
    return [
        FeatureVariant(
        ...
        FeatureVariant(
            name="step2c_plus_selected_lineup_44_features",
            description="Step2C+: Step2A + Step2C重要度上位から選択した12特徴量。",
            categorical_features=score_cat,
            numeric_features=step2c_plus_num,
        ),
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_step2c_plus_lightgbm_selected_lineup_structure",
        help="出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("Step 2C+: LightGBM selected enhanced lineup-structure feature comparison")
    print("=" * 80)
    ...
    coverage_rows = []
    for col in unique_features(LINEUP_SUPPORT_NUMERIC_FEATURES + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES + STEP2C_ALL_ENGINEERED_NUMERIC_FEATURES + LINEUP_DIAGNOSTIC_FEATURES):
    ...
    coverage_df.to_csv(output_dir / "step2c_plus_lineup_feature_coverage.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(summary_rows).to_csv(output_dir / "step2c_plus_feature_variant_summary.csv", index=False, encoding="utf-8-sig")
        ...
        json.dump(
            {
                ...
                "step2c_plus_added_numeric_features": STEP2C_PLUS_ADDED_NUMERIC_FEATURES,
    ...
    comparison_df.to_csv(output_dir / "step2c_plus_lightgbm_selected_lineup_structure_comparison_metrics.csv", index=False, encoding="utf-8-sig")

    print("\n" + "=" * 80)
    print("Step 2C+ comparison")
    print("=" * 80)
    print(comparison_df.to_string(index=False))
    print(f"\nSaved: {output_dir / 'step2c_plus_lightgbm_selected_lineup_structure_comparison_metrics.csv'}")
    print("Done.")
