...
実行例:
python evaluate_step4_threshold_tuning_step2a.py --inning_csv_path "C:/Users/Admin/Desktop/NPB/evaluate_inning/data/train_inning_runs_added_9_features.csv" --pa_csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv"
...
# ============================================================
# Step 4: Threshold tuning for LightGBM Step2A and FM binned
# ============================================================

THRESHOLD_GRID = np.round(np.arange(0.05, 0.951, 0.01), 2)


def evaluate_at_thresholds(y_true: np.ndarray, y_prob: np.ndarray, thresholds: np.ndarray) -> pd.DataFrame:
    rows = []
    for th in thresholds:
        y_pred = (y_prob >= float(th)).astype(int)
        metrics = compute_binary_metrics(y_true, y_pred, y_prob)
        rows.append({"threshold": float(th), **metrics})
    return pd.DataFrame(rows)


def choose_thresholds(valid_curve: pd.DataFrame) -> Dict[str, float]:
    """valid上で代表的な最適閾値を選ぶ。"""
    choices: Dict[str, float] = {}
    # 1) Macro F1最大: 全体バランス重視
    idx = valid_curve["macro_f1"].idxmax()
    choices["best_macro_f1"] = float(valid_curve.loc[idx, "threshold"])
    # 2) RUN_SCORED F1最大: 得点イニング検出のF1重視
    idx = valid_curve["run_scored_f1"].idxmax()
    choices["best_run_scored_f1"] = float(valid_curve.loc[idx, "threshold"])
    # 3) RUN_SCORED Recall >= 0.50 を満たす中でRUN_SCORED F1最大
    cand = valid_curve[valid_curve["run_scored_recall"] >= 0.50]
    if len(cand) > 0:
        idx = cand["run_scored_f1"].idxmax()
        choices["best_run_f1_recall_ge_050"] = float(cand.loc[idx, "threshold"])
    # 4) RUN_SCORED Recall >= 0.60 を満たす中でRUN_SCORED F1最大
    cand = valid_curve[valid_curve["run_scored_recall"] >= 0.60]
    if len(cand) > 0:
        idx = cand["run_scored_f1"].idxmax()
        choices["best_run_f1_recall_ge_060"] = float(cand.loc[idx, "threshold"])
    return choices
...
def metrics_for_selected_thresholds(
    ...
    """validで閾値選択し、testに適用した評価を返す。"""
    model_dir = output_dir / model_name
    model_dir.mkdir(parents=True, exist_ok=True)

    valid_curve = evaluate_at_thresholds(y_valid, valid_prob, THRESHOLD_GRID)
    test_curve = evaluate_at_thresholds(y_test, test_prob, THRESHOLD_GRID)
    valid_curve.to_csv(model_dir / f"{model_name}_valid_threshold_curve.csv", index=False, encoding="utf-8-sig")
    test_curve.to_csv(model_dir / f"{model_name}_test_threshold_curve.csv", index=False, encoding="utf-8-sig")

    selected = {"default_0_50": 0.50}
    selected.update(choose_thresholds(valid_curve))

    rows = []
    for criterion, th in selected.items():
        valid_pred = (valid_prob >= th).astype(int)
        test_pred = (test_prob >= th).astype(int)
        valid_metrics = compute_binary_metrics(y_valid, valid_pred, valid_prob)
        ...
        # 保存: 選択閾値ごとのtest混同行列・予測
        safe_criterion = criterion.replace("/", "_")
        cm = confusion_matrix(y_test, test_pred, labels=[0, 1])
        pd.DataFrame(cm, index=LABELS, columns=LABELS).to_csv(
            model_dir / f"{model_name}_{safe_criterion}_threshold_{th:.2f}_confusion_matrix.csv",
            encoding="utf-8-sig",
        )
        pred_df = pd.DataFrame({
            "game_date": test_df["game_date"].values,
            ...
            "pred_label": [ID_TO_LABEL[int(x)] for x in test_pred],
            "prob_RUN_SCORED": test_prob,
            "threshold": float(th),
            "selection_criterion": criterion,
        })
        pred_df.to_csv(
            model_dir / f"{model_name}_{safe_criterion}_threshold_{th:.2f}_predictions.csv",
            index=False,
            encoding="utf-8-sig",
        )

    selected_df = pd.DataFrame(rows)
    selected_df.to_csv(model_dir / f"{model_name}_selected_threshold_metrics.csv", index=False, encoding="utf-8-sig")
    return selected_df, valid_curve


def train_lightgbm_get_probs(split: SplitData, variant: FeatureVariant, output_dir: Path, seed: int):
    try:
        import lightgbm as lgb
    except ImportError as e:
        raise ImportError("lightgbm が必要です。pip install lightgbm を実行してください。") from e
    ...
    print("\n" + "-" * 80)
    print(f"LightGBM training for threshold tuning: variant={variant.name}")
    print("-" * 80)
    variant_dir = output_dir / variant.name
    variant_dir.mkdir(parents=True, exist_ok=True)
    save_feature_list(variant_dir, variant)
    ...
    return y_valid, valid_prob, y_test, test_prob, test_df, info


def train_fm_get_probs(split: SplitData, variant: FeatureVariant, output_dir: Path, args):
    print("\n" + "-" * 80)
    print(f"FM training for threshold tuning: variant={variant.name}")
    print("-" * 80)
    print(f"categorical_features ({len(variant.categorical_features)}): {variant.categorical_features}")
    print(f"numeric_features ({len(variant.numeric_features)}): {variant.numeric_features}")
    ...
    model, info = train_fm_model(
        ...
        max_epochs=args.fm_epochs,
        batch_size=args.batch_size,
        patience=args.patience,
        lr=args.lr,
        embed_dim=args.embed_dim,
    )
    valid_prob = predict_fm(model, X_cat_valid, X_num_valid, batch_size=args.batch_size)
    test_prob = predict_fm(model, X_cat_test, X_num_test, batch_size=args.batch_size)
    ...
    return y_valid, valid_prob, y_test, test_prob, test_df, model_info


def make_step4_variants() -> Tuple[FeatureVariant, FeatureVariant]:
    lgbm_step2a = FeatureVariant(
        name="lgbm_step2a_raw_32_features",
        model="lightgbm",
        categorical_features=STEP2A_CATEGORICAL_FEATURES,
        numeric_features=STEP2A_NUMERIC_FEATURES,
        description="LightGBM Step2A raw 32 features. Threshold is tuned on valid.",
    )
    fm_binned = FeatureVariant(
        name="fm_step2a_binned_features",
        model="fm",
        categorical_features=BINNED_CATEGORICAL_FEATURES,
        numeric_features=[],
        description="FM Step2A binned features. Threshold is tuned on valid.",
    )
    return lgbm_step2a, fm_binned
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=str, default="output_step4_threshold_tuning_step2a")
    ...
    print("=" * 80)
    print("Step 4: Threshold tuning for LightGBM Step2A and FM binned Step2A")
    print("=" * 80)
    print(f"inning_csv_path: {inning_csv_path}")
    print(f"pa_csv_path: {pa_csv_path}")
    print(f"output_dir: {output_dir}")
    print("target: NO_RUN / RUN_SCORED")
    print("threshold grid: 0.05 ~ 0.95 by 0.01")
    ...
    if not args.no_save_augmented_csv:
        df.to_csv(output_dir / "train_inning_runs_step4_threshold_tuning_step2a.csv", index=False, encoding="utf-8-sig")
    ...
    lgbm_variant, fm_variant = make_step4_variants()
    variants = []
    if not args.skip_lightgbm:
        variants.append(lgbm_variant)
    if not args.skip_fm:
        variants.append(fm_variant)
    ...
    pd.DataFrame(summary_rows).to_csv(output_dir / "step4_feature_variant_summary.csv", index=False, encoding="utf-8-sig")

    with open(output_dir / "feature_config_step4.json", "w", encoding="utf-8") as f:
        json.dump({
            "step2a_categorical_features": STEP2A_CATEGORICAL_FEATURES,
            "step2a_numeric_features": STEP2A_NUMERIC_FEATURES,
            "fm_binned_categorical_features": BINNED_CATEGORICAL_FEATURES,
            "threshold_grid": THRESHOLD_GRID.tolist(),
            "selection_criteria": ["default_0_50", "best_macro_f1", "best_run_scored_f1", "best_run_f1_recall_ge_050", "best_run_f1_recall_ge_060"],
    ...
    if not args.skip_lightgbm:
        yv, pv, yt, pt, test_df, info = train_lightgbm_get_probs(split, lgbm_variant, output_dir, args.seed)
        model_info_rows.append(info)
        selected_df, _ = metrics_for_selected_thresholds(lgbm_variant.name, yv, pv, yt, pt, output_dir, test_df)
        selected_rows.append(selected_df)

    if not args.skip_fm:
        yv, pv, yt, pt, test_df, info = train_fm_get_probs(split, fm_variant, output_dir, args)
        model_info_rows.append(info)
        selected_df, _ = metrics_for_selected_thresholds(fm_variant.name, yv, pv, yt, pt, output_dir, test_df)
        selected_rows.append(selected_df)

    model_info = pd.DataFrame(model_info_rows)
    model_info.to_csv(output_dir / "step4_model_training_info.csv", index=False, encoding="utf-8-sig")

    comp = pd.concat(selected_rows, ignore_index=True) if selected_rows else pd.DataFrame()
    preferred_cols = [
        "model", "selection_criterion", "threshold",
        "valid_accuracy", "valid_macro_f1", "valid_run_scored_precision", "valid_run_scored_recall", "valid_run_scored_f1", "valid_run_scored_pred_count", "valid_roc_auc", "valid_pr_auc",
        "test_accuracy", "test_macro_f1", "test_weighted_f1", "test_no_run_precision", "test_no_run_recall", "test_no_run_f1",
        "test_run_scored_precision", "test_run_scored_recall", "test_run_scored_f1", "test_run_scored_pred_count", "test_log_loss", "test_roc_auc", "test_pr_auc",
    ]
    cols = [c for c in preferred_cols if c in comp.columns] + [c for c in comp.columns if c not in preferred_cols]
    comp = comp[cols]
    comp.to_csv(output_dir / "step4_threshold_tuning_comparison_metrics.csv", index=False, encoding="utf-8-sig")

    print("\n" + "=" * 80)
    print("Step 4 threshold tuning comparison")
    print("=" * 80)
    print(comp.to_string(index=False))
    print(f"\nSaved: {output_dir / 'step4_threshold_tuning_comparison_metrics.csv'}")
    print("Done.")
