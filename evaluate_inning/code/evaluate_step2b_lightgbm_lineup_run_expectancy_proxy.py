# -*- coding: utf-8 -*-
"""
Step 2B: LightGBM inning-level RUN_SCORED prediction with lineup run-expectancy proxy features.
...
実行例:
python evaluate_step2b_lightgbm_lineup_run_expectancy_proxy.py --inning_csv_path "C:/Users/Admin/Desktop/NPB/evaluate_inning/data/train_inning_runs_added_9_features.csv" --pa_csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv"
python evaluate_step2b_lightgbm_lineup_run_expectancy_proxy.py \
...
# Step2B features: inning-start run-expectancy proxy from leadoff and next 3 hitters
STEP2B_RUN_EXPECTANCY_NUMERIC_FEATURES = [
...
def add_lineup_strength_features(inning_df: pd.DataFrame, pa_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """通常の打線強度・投手左右対応・Step2B得点期待proxy特徴量を追加する。"""
    ...
    def expected_count_from_rates(values: List[float], n_expected: int = 3) -> float:
        """率の平均×人数として、次のn人における期待発生数を作る。"""
        m = nanmean_or_nan(values)
        if np.isnan(m):
            return np.nan
        return float(m * n_expected)
    ...
    for row in df.itertuples(index=False):
        ...
        out = {
            ...
            # Step2B: leadoff and next-3 run-expectancy proxy
            **leadoff_vals,
            ...
            "expected_onbase_next3": expected_count_from_rates(vals3_obp, 3),
            "expected_bb_next3": expected_count_from_rates(vals3_bb, 3),
            "expected_k_next3": expected_count_from_rates(vals3_k, 3),
            "expected_xbh_next3": expected_count_from_rates(vals3_xbh, 3),
            # diagnostics
...
def prepare_inning_dataframe(inning_csv_path: Path, pa_csv_path: Path, output_dir: Path, save_augmented_csv: bool = True) -> pd.DataFrame:
    ...
    numeric_features = BASE_NUMERIC_FEATURES + SCORE_CONTEXT_NUMERIC_FEATURES + LINEUP_STRENGTH_NUMERIC_FEATURES + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES + STEP2B_RUN_EXPECTANCY_NUMERIC_FEATURES + LINEUP_DIAGNOSTIC_FEATURES
    ...
    lineup_table.to_csv(output_dir / "step2b_lineup_slot_table_from_pa.csv", index=False, encoding="utf-8-sig")

    if save_augmented_csv:
        df.to_csv(output_dir / "train_inning_runs_step2b_lineup_run_expectancy_proxy.csv", index=False, encoding="utf-8-sig")
...
def get_feature_variants() -> List[FeatureVariant]:
    ...
    step2b_num = score_num + STEP2B_RUN_EXPECTANCY_NUMERIC_FEATURES
    step2a_plus_step2b_num = unique_features(score_num + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES + STEP2B_RUN_EXPECTANCY_NUMERIC_FEATURES)
    return [
        FeatureVariant(
        ...
        FeatureVariant(
            name="step2b_run_expectancy_proxy_42_features",
            description="Step2B: score context + 先頭打者/次の3打者の出塁・四球・三振・長打期待proxy。",
            categorical_features=score_cat,
            numeric_features=step2b_num,
        ),
        FeatureVariant(
            name="step2a_plus_step2b_46_features",
            description="Step2A + Step2B: 投手左右対応打線強度 + 得点期待proxyを同時投入。",
            categorical_features=score_cat,
            numeric_features=step2a_plus_step2b_num,
        ),
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_step2b_lightgbm_lineup_run_expectancy_proxy",
        help="出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("Step 2B: LightGBM lineup run-expectancy proxy feature comparison")
    print("=" * 80)
    ...
    coverage_rows = []
    for col in unique_features(LINEUP_STRENGTH_NUMERIC_FEATURES + LINEUP_VS_PITCHER_HAND_NUMERIC_FEATURES + STEP2B_RUN_EXPECTANCY_NUMERIC_FEATURES + LINEUP_DIAGNOSTIC_FEATURES):
    ...
    coverage_df.to_csv(output_dir / "step2b_lineup_feature_coverage.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(summary_rows).to_csv(output_dir / "step2b_feature_variant_summary.csv", index=False, encoding="utf-8-sig")
        ...
        json.dump(
            {
                ...
                "step2b_run_expectancy_numeric_features": STEP2B_RUN_EXPECTANCY_NUMERIC_FEATURES,
    ...
    comparison_df.to_csv(output_dir / "step2b_lightgbm_lineup_run_expectancy_proxy_comparison_metrics.csv", index=False, encoding="utf-8-sig")

    print("\n" + "=" * 80)
    print("Step 2B comparison")
    print("=" * 80)
    print(comparison_df.to_string(index=False))
    print(f"\nSaved: {output_dir / 'step2b_lightgbm_lineup_run_expectancy_proxy_comparison_metrics.csv'}")
    print("Done.")
