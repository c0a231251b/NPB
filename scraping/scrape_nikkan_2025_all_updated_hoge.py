...
class CalendarNikkanScraper:
    ...
    def parse_game_page(self, url):
        ...
        try:
            ...
            if "cl2025032901" in url:
                print("\n=== cl2025032901 ===")
                print("FIRST :", first_block_pitchers)
                print("SECOND:", second_block_pitchers)

            if "cl2025032902" in url:
                print("\n=== cl2025032902 ===")
                print("FIRST :", first_block_pitchers)
                print("SECOND:", second_block_pitchers)
            if "cl2025032903" in url:
                print("\n=== cl2025032903 ===")
                print("FIRST :", first_block_pitchers)
                print("SECOND:", second_block_pitchers) 
            ...
            for table in batter_tables:
                ...
                if start_idx is not None:
                    # 問題①②対策：start_idx以降だけをループし、if/elifをループ内に収める
                    for idx in range(start_idx, len(header_ths)):
                        txt = unicodedata.normalize(
                            "NFKC",
                            header_ths[idx].get_text().replace("\xa0", "").strip()
                        )
                        if txt.isdigit():
                            current_inning = int(txt)
                            inning_mapping.append((idx, current_inning))
                        elif current_inning is not None:
                            # 空ヘッダー = 同一イニング2打席目
                            inning_mapping.append((idx, current_inning))
                ...
                for th_absolute_idx, real_inning_num in inning_mapping:
                    for row in rows:
                        ...
                        res_text = target_col.get_text(" ", strip=True)

                        if not res_text:
                            continue

                        if res_text not in ["……", "---"] and re.search(r'[\u4e00-\u9fff\u30a0-\u30ff]', res_text):
                        ...
                        """
                        if "cl2025032901" in url and real_inning_num == 9 and side_label == "表":
                            print(
                                p_name,
                                [c.get_text(strip=True) for c in cols[:10]]
                            )
                            print(row)
