...
class FactorizationMachineModel(nn.Module):
    def __init__(self, num_players, k=16, num_num_features=2): # 2次元に修正
...
"""
入力ファイルを変更するだけで、得点差予測用のFMモデルが学習できるようにしたスクリプトです。
- fm_dataset_30d.pkl
- fm_dataset_30d_OBP.pkl
- fm_dataset_30d_SLG.pkl
- fm_dataset_30d_OPS.pkl
- fm_dataset_30d_ISO.pkl
- fm_dataset_30d_K.pkl
- fm_dataset_30d_BB.pkl
- fm_dataset_30d_HR.pkl
"""
def train_fm():
    df = pd.read_pickle("fm_dataset_30d_HR.pkl")
    X_cat = np.stack(df["cat_features"].values)
    X_num = np.stack(df["num_features"].values)
    y = df["target"].values.astype(np.float32)

    """
    入力ファイルを変更した場合それに合わせて、scalerの保存ファイル名も変更してください。
    - fm_dataset_30d.pkl -> fm_scaler_30d.gz
    - fm_dataset_30d_OBP.pkl -> fm_scaler_30d_OBP.gz
    - fm_dataset_30d_SLG.pkl -> fm_scaler_30d_SLG.gz
    - fm_dataset_30d_OPS.pkl -> fm_scaler_30d_OPS.gz
    - fm_dataset_30d_ISO.pkl -> fm_scaler_30d_ISO.gz
    - fm_dataset_30d_K.pkl -> fm_scaler_30d_K.gz
    - fm_dataset_30d_BB.pkl -> fm_scaler_30d_BB.gz
    - fm_dataset_30d_HR.pkl -> fm_scaler_30d_HR.gz
    """
    scaler = StandardScaler()
    X_num = scaler.fit_transform(X_num)
    joblib.dump(scaler, "fm_scaler_30d_HR.gz")
    ...
    model = FactorizationMachineModel(num_players=697, k=16, num_num_features=2)
    ...
    torch.save(model.state_dict(), "fm_model_30d_HR.pth")
    print("学習完了。 fm_model_30d_HR.pth を保存しました。")
