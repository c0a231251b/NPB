...
|batter_type_lstm.py|打者タイプ分類フラグ＋投手特徴量の13次元入力でLSTM得点予測を行い、学習曲線で過学習を監視するスクリプト|
...
|aggregate_real_batting_orders.py|試合JSONからスタメン打順を抽出し、選手クラスターと実際の起用打順の相関をヒートマップで可視化するスクリプト|
