# -*- coding: utf-8 -*-
"""
train_all_added_74_features.csv を用いて、提案3の4分類（SINGLE / XBH / BB_HBP / SO_OUT）で LightGBM と Factorization Machine(FM) を評価するスクリプト。

目的:
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm_67_features/model_comparison_metrics.csv に保存する

実行例:
python evaluate_lightgbm_fm_74_features_4class_so_out_class_weight.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_74_features.csv"
...
CLASS_SPECIFIC_MATCHUP_NUMERIC_FEATURES = [
    "batter_single_rate",
    "pitcher_single_rate_allowed",
    "single_matchup_index",
    "bb_matchup_index",
    "xbh_matchup_index",
]
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="data/train_all_added_74_features.csv",
        help="74特徴量を追加したCSVファイルパス",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_74_features_4class_so_out_class_weight",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 74-feature CSV / 4-class SO_OUT target / class_weight")
    print("=" * 80)
        ...
        json.dump(
            {
                ...
                "class_specific_matchup_numeric_features": CLASS_SPECIFIC_MATCHUP_NUMERIC_FEATURES,
                ...
                "added_from_69_features": CLASS_SPECIFIC_MATCHUP_NUMERIC_FEATURES,
