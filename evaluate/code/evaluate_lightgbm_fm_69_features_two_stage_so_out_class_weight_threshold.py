...
実行例:
python evaluate_lightgbm_fm_69_features_two_stage_so_out_class_weight_threshold.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv"

ON_BASE recallをより重視したい場合は、次も試せます。
python evaluate_lightgbm_fm_69_features_two_stage_so_out_class_weight_threshold.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv" --threshold_metric stage1_onbase_f1

必要ライブラリ:
    pip install pandas numpy scikit-learn lightgbm torch tqdm

出力:
- output_lgbm_fm_69_features_two_stage_so_out_class_weight_threshold/model_comparison_metrics.csv
...
def apply_stage1_onbase_threshold(
    p_stage1: np.ndarray,
    p_onbase: np.ndarray,
    p_out: np.ndarray,
    threshold: float,
) -> Tuple[np.ndarray, np.ndarray]:
    """Stage1のON_BASE確率にしきい値を適用し、最終5分類の予測と確率を作る。"""
    n = len(p_stage1)
    final_prob = np.zeros((n, len(FINAL_LABELS)), dtype=np.float32)
    final_pred = np.zeros(n, dtype=np.int64)

    p_on = p_stage1[:, STAGE1_LABEL_TO_ID["ON_BASE"]]
    use_onbase = p_on >= threshold

    on_pred = np.argmax(p_onbase, axis=1)
    on_to_final = np.array([
        FINAL_LABEL_TO_ID["SINGLE"],
        FINAL_LABEL_TO_ID["XBH"],
        FINAL_LABEL_TO_ID["BB_HBP"],
    ], dtype=np.int64)

    out_pred = np.argmax(p_out, axis=1)
    out_to_final = np.array([
        FINAL_LABEL_TO_ID["SO"],
        FINAL_LABEL_TO_ID["OTHER_OUT"],
    ], dtype=np.int64)

    final_pred[use_onbase] = on_to_final[on_pred[use_onbase]]
    final_pred[~use_onbase] = out_to_final[out_pred[~use_onbase]]

    final_prob[use_onbase, FINAL_LABEL_TO_ID["SINGLE"]] = p_onbase[use_onbase, ONBASE_LABEL_TO_ID["SINGLE"]]
    final_prob[use_onbase, FINAL_LABEL_TO_ID["XBH"]] = p_onbase[use_onbase, ONBASE_LABEL_TO_ID["XBH"]]
    final_prob[use_onbase, FINAL_LABEL_TO_ID["BB_HBP"]] = p_onbase[use_onbase, ONBASE_LABEL_TO_ID["BB_HBP"]]
    final_prob[~use_onbase, FINAL_LABEL_TO_ID["SO"]] = p_out[~use_onbase, OUT_LABEL_TO_ID["SO"]]
    final_prob[~use_onbase, FINAL_LABEL_TO_ID["OTHER_OUT"]] = p_out[~use_onbase, OUT_LABEL_TO_ID["OTHER_OUT"]]
    return final_pred, final_prob


def search_stage1_threshold(
    y_true: np.ndarray,
    p_stage1: np.ndarray,
    p_onbase: np.ndarray,
    p_out: np.ndarray,
    ...
    """validデータでStage1 ON_BASEしきい値を探索する。"""
    thresholds = np.round(np.arange(start, end + 1e-9, step), 10)
    rows = []
    for threshold in thresholds:
        y_pred, y_prob = apply_stage1_onbase_threshold(p_stage1, p_onbase, p_out, float(threshold))
        metrics = compute_metrics(y_true, y_pred, y_prob=y_prob)
        metrics["threshold"] = float(threshold)

        p_on = p_stage1[:, STAGE1_LABEL_TO_ID["ON_BASE"]]
        pred_on = p_on >= threshold
        true_on = np.isin(y_true, [FINAL_LABEL_TO_ID["SINGLE"], FINAL_LABEL_TO_ID["XBH"], FINAL_LABEL_TO_ID["BB_HBP"]])
        ...
        metrics["stage1_onbase_precision"] = float(precision)
        metrics["stage1_onbase_recall"] = float(recall)
        metrics["stage1_onbase_f1"] = float(f1)
        metrics["stage1_onbase_pred_count"] = int(np.sum(pred_on))
        rows.append(metrics)

    df = pd.DataFrame(rows)
    if metric_name not in df.columns:
        raise ValueError(f"threshold_metric={metric_name} は利用できません。利用可能: {list(df.columns)}")
    best_idx = df[metric_name].idxmax()
    best_threshold = float(df.loc[best_idx, "threshold"])
    df = df.sort_values(metric_name, ascending=False).reset_index(drop=True)
...
def train_evaluate_lightgbm_two_stage(
    split: SplitData,
    output_dir: Path,
    seed: int = 42,
    ...
    # 最終5分類確率へ合成: 通常の確率合成argmax
    p_stage1_valid = stage1_model.predict_proba(valid_df[feature_cols])
    p_onbase_valid = stage2_on_model.predict_proba(valid_df[feature_cols])
    p_out_valid = stage2_out_model.predict_proba(valid_df[feature_cols])
    ...
    y_valid = valid_df["final_target"].astype(int).values
    y_true = test_df["final_target"].astype(int).values

    final_prob_argmax = combine_two_stage_prob(p_stage1_test, p_onbase_test, p_out_test)
    final_pred_argmax = np.argmax(final_prob_argmax, axis=1)
    metrics_argmax = save_classification_outputs(
        output_dir, "lightgbm_two_stage_argmax", FINAL_LABELS, y_true, final_pred_argmax, final_prob_argmax
    )

    best_threshold, threshold_df = search_stage1_threshold(
        y_true=y_valid,
        p_stage1=p_stage1_valid,
        p_onbase=p_onbase_valid,
        p_out=p_out_valid,
    ...
    threshold_df.to_csv(output_dir / "lightgbm_stage1_threshold_search_valid.csv", index=False, encoding="utf-8-sig")

    print("\n[LightGBM Stage1 ON_BASE Threshold Search]")
    print(f"optimize_metric: {threshold_metric}")
    print(f"best_threshold: {best_threshold:.4f}")
    print("Top 10 thresholds on valid:")
    print(threshold_df.head(10).to_string(index=False))

    final_pred_th, final_prob_th = apply_stage1_onbase_threshold(
        p_stage1_test, p_onbase_test, p_out_test, best_threshold
    )
    metrics_threshold = save_classification_outputs(
        output_dir, "lightgbm_two_stage_threshold", FINAL_LABELS, y_true, final_pred_th, final_prob_th
    )
    metrics_threshold["best_threshold"] = float(best_threshold)
    metrics_threshold["optimized_on"] = f"valid_{threshold_metric}"

    print("\n[LightGBM Two-stage Final Test Metrics: argmax]")
    for k, v in metrics_argmax.items():
        if k != "model":
            print(f"{k}: {v}")

    print("\n[LightGBM Two-stage Final Test Metrics: Stage1 ON_BASE threshold]")
    for k, v in metrics_threshold.items():
        if k != "model":
            print(f"{k}: {v}")

    return [metrics_argmax, metrics_threshold]
...
def train_evaluate_fm_two_stage(
    ...
    # 全valid/test行に対する第2段階確率を計算して最終5分類へ合成
    p_stage1_valid = predict_fm_proba(stage1_model, valid_df, stage1_encoder, device, batch_size=batch_size)
    p_onbase_valid = predict_fm_proba(on_model, valid_df, on_encoder, device, batch_size=batch_size)
    p_out_valid = predict_fm_proba(out_model, valid_df, out_encoder, device, batch_size=batch_size)
    ...
    final_prob_argmax = combine_two_stage_prob(p_stage1, p_onbase, p_out)
    final_pred_argmax = np.argmax(final_prob_argmax, axis=1)
    metrics_argmax = save_classification_outputs(
        output_dir, "fm_two_stage_argmax", FINAL_LABELS, y_true, final_pred_argmax, final_prob_argmax
    )
    ...
    threshold_df.to_csv(output_dir / "fm_stage1_threshold_search_valid.csv", index=False, encoding="utf-8-sig")

    print("\n[FM Stage1 ON_BASE Threshold Search]")
    print(f"optimize_metric: {threshold_metric}")
    print(f"best_threshold: {best_threshold:.4f}")
    print("Top 10 thresholds on valid:")
    print(threshold_df.head(10).to_string(index=False))
    ...
    final_pred_th, final_prob_th = apply_stage1_onbase_threshold(
        p_stage1, p_onbase, p_out, best_threshold
    )
    metrics_threshold = save_classification_outputs(
        output_dir, "fm_two_stage_threshold", FINAL_LABELS, y_true, final_pred_th, final_prob_th
    )

    for metrics in (metrics_argmax, metrics_threshold):
    ...
    print("\n[FM Two-stage Final Test Metrics: argmax]")
    for k, v in metrics_argmax.items():
        if k != "model":
            print(f"{k}: {v}")

    print("\n[FM Two-stage Final Test Metrics: Stage1 ON_BASE threshold]")
    for k, v in metrics_threshold.items():
        if k != "model":
            print(f"{k}: {v}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_69_features_two_stage_so_out_class_weight_threshold",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    parser.add_argument("--threshold_metric", type=str, default="macro_f1", help="Stage1しきい値探索で最大化するvalid指標")
    parser.add_argument("--threshold_start", type=float, default=0.05, help="しきい値探索の開始値")
    parser.add_argument("--threshold_end", type=float, default=0.95, help="しきい値探索の終了値")
    parser.add_argument("--threshold_step", type=float, default=0.01, help="しきい値探索の刻み幅")
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 69-feature CSV / two-stage target / class_weight / Stage1 ON_BASE threshold")
    print("=" * 80)
        ...
        json.dump(
            {
                ...
                "stage1_threshold": {
                    "target_probability": "ON_BASE",
                    "rule": "if prob_ON_BASE >= threshold then use Stage2 ON_BASE else use Stage2 OUT",
                    "selected_by": "valid final classification metric",
                },
            },
            f,
            ensure_ascii=False,
    ...
    if not args.skip_lightgbm:
        lgb_metrics_list = train_evaluate_lightgbm_two_stage(
            split,
            output_dir,
            seed=args.seed,
        ...
        if lgb_metrics_list is not None:
            all_metrics.extend(lgb_metrics_list)

    if not args.skip_fm:
        fm_metrics_list = train_evaluate_fm_two_stage(
            split,
            output_dir,
            seed=args.seed,
        ...
        all_metrics.extend(fm_metrics_list)
