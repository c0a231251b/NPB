# -*- coding: utf-8 -*-
"""
train_all_added_37_features.csv を用いて LightGBM と Factorization Machine(FM) を同時に評価するスクリプト。

目的:
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm_37_features/model_comparison_metrics.csv に保存する

実行例:
python evaluate_lightgbm_fm_37_features.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_37_features.csv"
...
- 既存特徴量 + 左右別成績8特徴量 + 直近成績9特徴量 + 試合状況3特徴量 + 累積率10特徴量を扱います。27特徴量版に batter_k_rate / batter_bb_rate / batter_hr_rate / batter_xbh_rate / batter_pa_count / pitcher_k_rate / pitcher_bb_rate / pitcher_hr_rate / pitcher_xbh_rate_allowed / pitcher_bf_count を追加した37特徴量版です。
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_37_features",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 37-feature CSV")
    print("=" * 80)
