# -*- coding: utf-8 -*-
"""
train_all_added_69_features.csv を用いて、提案3の4分類（SINGLE / XBH / BB_HBP / SO_OUT）で LightGBM と Factorization Machine(FM) を評価するスクリプト。

目的:
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm_69_features/model_comparison_metrics.csv に保存する

実行例:
python evaluate_lightgbm_fm_69_features_4class_so_out_class_weight_threshold.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv"
しきい値の最適化指標を変える場合は以下の通り
python evaluate_lightgbm_fm_69_features_4class_so_out_class_weight_threshold.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_69_features.csv" --threshold_metric non_so_out_f1
...
- 既存特徴量 + 左右別成績8特徴量 + 直近成績9特徴量 + 試合状況3特徴量 + 累積率10特徴量 + イベント/場面6特徴量 + 出塁/直接対戦/状況12特徴量を扱います。43特徴量版に batter_onbase_event_rate / pitcher_onbase_event_rate_allowed / vs_pa_count / vs_hit_rate / vs_k_rate / vs_bb_rate / vs_xbh_rate / inning_out_state / bat_order_runner_state / is_no_out_runner_on / is_two_out / month を追加した55特徴量版をベースに、さらに打球アウト傾向・直接対戦本塁打率・打順カテゴリ・前回出場/登板間隔・同一試合内の同一打者対戦回数・チームIDを追加した69特徴量版です.
"""
...
def apply_so_out_threshold(y_prob: np.ndarray, threshold: float) -> np.ndarray:
    """
    SO_OUTに寄りすぎるargmaxを補正する。

    ルール:
    - prob_SO_OUT >= threshold の場合は SO_OUT
    - それ以外の場合は SINGLE / XBH / BB_HBP の中で最大確率のクラスを選ぶ

    thresholdを高くすると、SO_OUT予測が減り、少数クラス予測が増えやすい。
    thresholdを低くすると、SO_OUT予測が増え、通常のargmaxに近づきやすい。
    """
    so_out_idx = LABEL_TO_ID["SO_OUT"]
    non_so_out_indices = [LABEL_TO_ID["SINGLE"], LABEL_TO_ID["XBH"], LABEL_TO_ID["BB_HBP"]]

    y_pred = np.empty(y_prob.shape[0], dtype=np.int64)
    so_mask = y_prob[:, so_out_idx] >= threshold
    y_pred[so_mask] = so_out_idx

    non_so_probs = y_prob[~so_mask][:, non_so_out_indices]
    if non_so_probs.shape[0] > 0:
        y_pred[~so_mask] = np.array(non_so_out_indices, dtype=np.int64)[np.argmax(non_so_probs, axis=1)]

    return y_pred


def calc_non_so_out_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    """SINGLE / XBH / BB_HBP をまとめた NON_SO_OUT 検出性能を計算する。"""
    so_out_idx = LABEL_TO_ID["SO_OUT"]
    true_non = y_true != so_out_idx
    pred_non = y_pred != so_out_idx
...
def search_so_out_threshold(
    y_true: np.ndarray,
    y_prob: np.ndarray,
    thresholds: np.ndarray,
    optimize_metric: str = "macro_f1",
) -> Tuple[float, pd.DataFrame]:
    """validデータでSO_OUTしきい値を探索する。"""
    rows = []
    for threshold in thresholds:
        y_pred = apply_so_out_threshold(y_prob, float(threshold))
        metrics = compute_metrics(y_true, y_pred, y_prob=y_prob)
        non_metrics = calc_non_so_out_metrics(y_true, y_pred)
        row = {
    ...
    if optimize_metric not in result_df.columns:
        raise ValueError(f"optimize_metric が不正です: {optimize_metric}")

    # 同点の場合はAccuracyも高いものを優先し、それでも同じならthresholdが小さいものを優先
    sort_cols = [optimize_metric, "accuracy", "weighted_f1"]
    result_df = result_df.sort_values(sort_cols, ascending=[False, False, False]).reset_index(drop=True)
    best_threshold = float(result_df.loc[0, "threshold"])
    return best_threshold, result_df


def add_non_so_out_metrics(metrics: Dict[str, float], y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    """通常metricsにNON_SO_OUT検出性能を追加する。"""
    metrics.update(calc_non_so_out_metrics(y_true, y_pred))
    return metrics

def make_class_weights(y_train: np.ndarray) -> np.ndarray:
    """学習データの分布から balanced class weight を計算する。"""
...
def save_report(
    ...
    metrics = add_non_so_out_metrics(metrics, y_true, y_pred)
    return metrics
...
def train_evaluate_lightgbm(split: SplitData, output_dir: Path, seed: int = 42, threshold_step: float = 0.01, optimize_metric: str = "macro_f1") -> Optional[List[Dict[str, float]]]:
    """LightGBMを学習・評価する。"""
    try:
    ...
    metrics_argmax = save_report(output_dir, "lightgbm_argmax", y_test, y_pred, y_prob)

    print("\n[LightGBM Test Metrics: argmax]")
    for k, v in metrics_argmax.items():
        if k != "model":
            print(f"{k}: {v}")

    # ------------------------------
    # 2) validでSO_OUTしきい値探索 → testへ適用
    # ------------------------------
    valid_prob = model.predict_proba(X_valid)
    thresholds = np.round(np.arange(0.05, 0.951, threshold_step), 4)
    best_threshold, threshold_df = search_so_out_threshold(
        y_valid,
        valid_prob,
        thresholds=thresholds,
        optimize_metric=optimize_metric,
    )
    threshold_df.to_csv(output_dir / "lightgbm_threshold_search_valid.csv", index=False, encoding="utf-8-sig")

    y_pred_threshold = apply_so_out_threshold(y_prob, best_threshold)
    metrics_threshold = save_report(output_dir, "lightgbm_threshold", y_test, y_pred_threshold, y_prob)
    metrics_threshold["best_threshold"] = float(best_threshold)
    metrics_threshold["optimized_on"] = f"valid_{optimize_metric}"

    print("\n[LightGBM SO_OUT Threshold Search]")
    ...
    print("\n[LightGBM Test Metrics: SO_OUT threshold]")
...
def train_evaluate_fm(
    ...
    metrics_argmax = save_report(output_dir, "fm_argmax", y_test, y_pred, y_prob)
    metrics_argmax["test_loss"] = float(test_loss)
    metrics_argmax["best_epoch"] = int(best_epoch)
    metrics_argmax["best_valid_loss"] = float(best_valid_loss)

    print("\n[FM Test Metrics: argmax]")
    for k, v in metrics_argmax.items():
        if k != "model":
            print(f"{k}: {v}")

    # validでSO_OUTしきい値探索 → testへ適用
    valid_loss, y_valid, pred_valid, prob_valid = evaluate_fm(model, valid_loader, criterion, device)
    thresholds = np.round(np.arange(0.05, 0.951, threshold_step), 4)
    best_threshold, threshold_df = search_so_out_threshold(
        y_valid,
    ...
    threshold_df.to_csv(output_dir / "fm_threshold_search_valid.csv", index=False, encoding="utf-8-sig")

    y_pred_threshold = apply_so_out_threshold(y_prob, best_threshold)
    metrics_threshold = save_report(output_dir, "fm_threshold", y_test, y_pred_threshold, y_prob)
    metrics_threshold["test_loss"] = float(test_loss)
    metrics_threshold["best_epoch"] = int(best_epoch)
    metrics_threshold["best_valid_loss"] = float(best_valid_loss)
    metrics_threshold["best_threshold"] = float(best_threshold)
    metrics_threshold["optimized_on"] = f"valid_{optimize_metric}"

    print("\n[FM SO_OUT Threshold Search]")
    print(f"optimize_metric: {optimize_metric}")
    print(f"best_threshold: {best_threshold:.4f}")
    print("Top 10 thresholds on valid:")
    print(threshold_df.head(10).to_string(index=False))

    print("\n[FM Test Metrics: SO_OUT threshold]")
    for k, v in metrics_threshold.items():
        if k != "model":
            print(f"{k}: {v}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_69_features_4class_so_out_class_weight_threshold",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="SO_OUTしきい値探索の刻み幅",
    )
    parser.add_argument(
        "--threshold_metric",
        type=str,
        default="macro_f1",
        choices=["accuracy", "macro_f1", "weighted_f1", "non_so_out_f1", "non_so_out_recall"],
        help="validデータで最適しきい値を選ぶ評価指標",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 69-feature CSV / 4-class SO_OUT target / class_weight / SO_OUT threshold")
    print("=" * 80)
        ...
        json.dump(
            {
                ...
                "added_from_55_features": BATTED_BALL_TEAM_NUMERIC_FEATURES + ["batter_team_id", "pitcher_team_id"],
                "added_from_67_features": INTERLEAGUE_SEASON_PROGRESS_NUMERIC_FEATURES,
    ...
    if not args.skip_lightgbm:
        lgb_metrics = train_evaluate_lightgbm(
            split,
            output_dir,
            seed=args.seed,
        ...
        if lgb_metrics is not None:
            all_metrics.extend(lgb_metrics)
