# -*- coding: utf-8 -*-
"""
train_all_added_37_features.csv を用いて、目的変数を4分類に再定義し、LightGBM と Factorization Machine(FM) を同時に評価するスクリプト。

目的:
- 37特徴量版を用いる
- 目的変数を HIT / BB_HBP / SO / OUT の4分類に変換する
- class_weight により少数クラスを重視して学習する
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm_37_features_4class_class_weight/model_comparison_metrics.csv に保存する

実行例:
    python evaluate_lightgbm_fm_37_features_4class_class_weight.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_37_features.csv"
...
- 37特徴量版の特徴量はそのまま使い、目的変数だけを4分類に再定義します。
- 4分類: HIT = SINGLE/DOUBLE/TRIPLE/HR, BB_HBP = BB/HBP, SO = SO, OUT = OUT
"""
...
def train_evaluate_fm(
    ...
    # trainデータのクラス分布から class_weight を計算する。
    # 少数クラスである BB_HBP / SO / HIT をより重視するために使用する。
    classes = np.arange(len(LABELS))
    train_y = split.train_df["target"].astype(int).values
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="data/train_all_added_37_features.csv",
        help="37特徴量を追加したCSVファイルパス",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_37_features_4class_class_weight",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 37-feature CSV / 4-class target / class_weight")
    print("=" * 80)
        ...
        json.dump(
            {
                ...
                "class_weight": {
                    "lightgbm": "balanced",
                    "fm": "balanced weights computed from train split",
                },
            },
            f,
            ensure_ascii=False,
