...
目的:
- 43特徴量版CSVを使い、SINGLE / XBH / BB_HBP / SO / OUT の5分類で評価する
...
- 比較結果を output_lgbm_fm_43_features_5class_class_weight/model_comparison_metrics.csv に保存する

実行例:
    python evaluate_lightgbm_fm_43_features_5class_class_weight.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_43_features.csv"
...
def convert_label(raw_label: str) -> str:
    """細かい打席結果を SINGLE / XBH / BB_HBP / SO / OUT の5クラスへ変換する。"""
...
def make_class_weights(y_train: np.ndarray) -> np.ndarray:
    """trainデータから5クラスのbalanced class weightを計算する。"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_43_features_5class_class_weight",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 43-feature CSV / 5-class target / class_weight")
    print("=" * 80)
