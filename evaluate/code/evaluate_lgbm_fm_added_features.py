# -*- coding: utf-8 -*-
"""
train_all_added_7_features.csv を用いて LightGBM と Factorization Machine(FM) を同時に評価するスクリプト。

目的:
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm/model_comparison_metrics.csv に保存する

実行例:
    python evaluate_lgbm_fm_added_features.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_7_features.csv"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="7特徴量を追加したCSVファイルパス",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm",
        help="評価結果の出力先ディレクトリ",
    )
