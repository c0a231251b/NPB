# -*- coding: utf-8 -*-
"""
train_all_added_24_features.csv を用いて LightGBM と Factorization Machine(FM) を同時に評価するスクリプト。

目的:
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm_24_features/model_comparison_metrics.csv に保存する

実行例:
    python evaluate_lightgbm_fm_24_features.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_24_features.csv"
...
- 既存特徴量 + 左右別成績8特徴量 + 直近成績9特徴量を数値特徴量として扱います。
"""
...
NUMERIC_FEATURES = BASE_NUMERIC_FEATURES + HAND_SPLIT_NUMERIC_FEATURES + RECENT_NUMERIC_FEATURES
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="data/train_all_added_24_features.csv",
        help="24特徴量を追加したCSVファイルパス",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_24_features",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 24-feature CSV")
    print("=" * 80)
