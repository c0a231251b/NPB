# -*- coding: utf-8 -*-
"""
train_all_added_55_features.csv を用いて LightGBM と Factorization Machine(FM) を同時に評価するスクリプト。

目的:
- 55特徴量版CSVを使い、HIT / BB_HBP / SO / OUT の4分類で評価する
- class_weight を導入し、OUT偏重を抑えて少数クラスのrecall改善を検証する
- 同一データ分割・同一特徴量で LightGBM と一般的なFMを比較する
- 評価指標として Accuracy / Macro F1 / Weighted F1 / Top-k Accuracy を出力する
- 比較結果を output_lgbm_fm_55_features_4class_class_weight/model_comparison_metrics.csv に保存する

実行例:
    python evaluate_lightgbm_fm_55_features_4class_class_weight.py --csv_path "C:/Users/Admin/Desktop/NPB/evaluate/data/train_all_added_55_features.csv"
...
- previous_result1〜5、base_out_state、inning_out_state、bat_order_runner_state はカテゴリ特徴量として扱います。
- LightGBM は class_weight="balanced" を使用します。
- FM は CrossEntropyLoss(weight=class_weights) を使用します。
"""
...
def convert_label(raw_label: str) -> str:
    """細かい打席結果を HIT / BB_HBP / SO / OUT の4クラスへ変換する。"""
    x = str(raw_label).strip()

    if x in SINGLE_SET or x in DOUBLE_SET or x in TRIPLE_SET or x in HR_SET:
        return "HIT"
...
def make_class_weights(y_train: np.ndarray) -> np.ndarray:
    """trainデータから4クラスのbalanced class weightを計算する。"""
...
class FactorizationMachineClassifier(nn.Module):
    ...
    def forward(self, x_cat: torch.Tensor, x_num: torch.Tensor) -> torch.Tensor:
        ...
        interaction = 0.5 * (summed_square - square_summed)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output_lgbm_fm_55_features_4class_class_weight",
        help="評価結果の出力先ディレクトリ",
    )
    ...
    print("=" * 80)
    print("LightGBM / FM comparison for 55-feature CSV / 4-class target / class_weight")
    print("=" * 80)
        ...
        json.dump(
            {
                ...
                "added_from_43_features": ONBASE_VS_CONTEXT_NUMERIC_FEATURES + [
                    "inning_out_state",
                    "bat_order_runner_state",
                ],
                "target_definition": {
